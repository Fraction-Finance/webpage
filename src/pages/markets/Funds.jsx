...
const Funds = () => {
  ...
  const categories = useMemo(() => {
    if (!funds) return [];
    const uniqueCategories = [...new Set(funds.map(fund => fund.stock_market_category).filter(Boolean))];
  ...
  const filteredFunds = useMemo(() => {
    ...
    if (activeCategory && activeCategory !== 'Todos') {
      categoryFiltered = funds.filter(sto => sto.stock_market_category === activeCategory);
    }
  ...
  }, [funds, searchTerm, activeCategory]);
