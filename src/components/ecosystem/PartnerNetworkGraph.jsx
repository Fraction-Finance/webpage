import React, { useState, useEffect, useMemo } from 'react';
    ...
    const PartnerNetworkGraph = () => {
      ...
      const categories = useMemo(() => [
        { name: 'Instituciones Financieras', angle: 45 },
        { name: 'Socios Tecnológicos', angle: 135 },
        { name: 'Regulación y Cumplimiento', angle: 225 },
        { name: 'Originadores de Activos', angle: 315 },
      ], []);
      ...
      const Line = ({ from, to }) => {
        ...
        return (
          <motion.div
            ...
            className="absolute h-[1px] bg-gray-300/70 origin-left"
            style={{
              width: `${length}px`,
              top: `${from.y}px`,
              left: `${from.x}px`,
      ...
      return (
        <div className="w-full h-[400px] md:h-[500px] flex items-center justify-center relative">
          ...
          >
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2" style={{width: `${graphRadius * 2.5}px`, height: `${graphRadius * 2.5}px`}}>
              {positions.filter(p => p.isPartner).map((pos, i) => {
                 ...
                 return <Line key={`line-partner-${i}`} from={{x: pos.x, y: pos.y}} to={{x: categoryPos.x, y: categoryPos.y}}/>
              })}
               {categories.map((cat, i) => {
                   const categoryPos = positions.find(p => p.name === cat.name);
                   if (!categoryPos) return null;
                   return <Line key={`line-cat-${i}`} from={{x: 0, y: 0}} to={categoryPos} />
               })}
            </div>

            <motion.div
              className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-10 w-24 h-24 bg-primary/10 rounded-full flex flex-col items-center justify-center text-primary shadow-lg glass-effect"
              whileHover={{ scale: 1.1 }}
            >
              <Users className="w-8 h-8" />
              <span className="text-sm font-bold mt-1">Tú</span>
            </motion.div>
            ...
            {positions.map((pos, i) => (
              ...
              >
                {pos.isPartner ? (
                   <motion.div 
                    className="w-12 h-12 rounded-full bg-white/80 glass-effect p-1 shadow-md flex items-center justify-center"
                  ...
                  <div className="w-28 h-12 rounded-lg bg-white/80 glass-effect shadow-md flex items-center justify-center text-center p-1">
